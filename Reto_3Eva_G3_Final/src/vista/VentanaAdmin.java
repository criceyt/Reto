...
public class VentanaAdmin extends JDialog implements ActionListener {
	...
	/**
	 * Campo de texto para el sueldo del trabajador.
	 */
	...
	/**
	 * Campo de contraseña para la entrada segura de contraseñas.
	 */
	...
	/**
	 * Etiqueta para el campo de texto del sueldo.
	 */
	...
	/**
	 * Etiqueta para el campo de texto de la contraseña.
	 */
	...
	/**
	 * Botón para insertar un trabajador.
	 */
	private JButton btnInsertarTrabajador;
	/**
	 * Botón para borrar datos.
	 */
	private JButton btnBorrarDatos_1;
	/**
	...
	/**
	 * Variable booleana para comprobar si el correo electrónico ha sido validado.
	 */
	private boolean boolComprobarEmail = true;
	/**
	 * Variable booleana para comprobar si el sueldo ha sido validado.
	 */
	private boolean boolComprobarSueldo = true;
	/**
	 * Variable booleana para comprobar si la contraseña ha sido validada.
	 */
	private boolean boolComprobarContrasena = true;
	/**
	 * Variable booleana para comprobar si la dirección ha sido validada.
	 */
	private boolean boolComprobarDireccion = true;
	/**
	...
	public VentanaAdmin(Controlador cont, InicioSesion padre, boolean modal) {
		...
		btnBorrarDatos_1 = new JButton("Borrar datos");
		btnBorrarDatos_1.addActionListener(new ActionListener() {
			...
			/**
			 * Método privado para limpiar o borrar los campos de texto en la interfaz de insertar trabajador.
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource().equals(btnBorrarDatos_1)) {
			txtDni.setText("");
			txtNombre.setText("");
			txtApellido.setText("");
			txtEdad.setText("");
		...
		if (e.getSource().equals(btnInsertarTrabajador)) {
			insertarTrabajador();
		}
	...
	/**
	 * Método privado para validar los datos ingresados por el usuario antes de insertar un nuevo trabajador en el sistema.
	 * Este método realiza una serie de validaciones en los campos de texto correspondientes a los datos del trabajador,
	 * como DNI, nombre, apellido, edad, correo electrónico, dirección, sueldo y contraseña, utilizando métodos de validación específicos.
	 * Si alguna de las validaciones falla, se cambia el color del texto de los labels correspondientes a rojo y se establece una variable booleana
	 * a false para indicar que los datos no son válidos. Si todas las validaciones pasan, se procede a verificar la longitud de la contraseña
	 * y, dependiendo del resultado, se ofrece al usuario la opción de cambiarla o continuar con el proceso de inserción.
	 * Si la contraseña es demasiado corta, se muestra un diálogo de alerta y se le pregunta al usuario si desea cambiarla.
	 * Si la contraseña es válida, se procede a insertar el trabajador en el sistema.
	 */
	private void insertarTrabajador() {
		// LO QUE VIENE ACONTINUACION ES UNA CADENA DE CONDICIENES QUE VALIDAN SI LO QUE
				...
				// Verificar si el campo de año de nacimiento está vacío o no contiene 4 números
				if (txtEdad.getText().isEmpty() || !validarEdad(txtEdad.getText().trim())) {
				...
				if (contrasenia.isEmpty()) {
					...
					boolComprobarContrasena = false;
				} else {
					lblContrasenia.setForeground(Color.WHITE);
					boolComprobarContrasena = true;
				}

				// Resultado Final De la inspeccion de datos
				if (!boolComprobarDNI | !boolComprobarNombre | !boolComprobarDireccion | !boolComprobarApellido | !boolComprobarEdad | !boolComprobarEmail | !boolComprobarSueldo | !boolComprobarContrasena) {
					ErrorContrasenaLogin.ErrorDeTrabajadorDatos();
				} else {
					...
					if (passwordField.getPassword().length < 8) {
						...
						if (option == JOptionPane.YES_OPTION) {
							...
							ErrorContrasenaLogin.MensajeAltaTrabajador();
							// Metodo de alta
	...
	/**
	 * Método privado para introducir un nuevo trabajador en el sistema.
	 * Este método crea una nueva instancia de la clase Trabajador, establece sus atributos con los datos ingresados por el usuario
	 * a través de los campos de texto correspondientes, y luego intenta agregar el trabajador al sistema
	 * Si ocurre algún error durante el proceso, se capturan las excepciones específicas y se imprime el stack trace para su depuración.
	 */
